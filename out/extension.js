...
function activate(context) {
    ...
    const disposable = vscode.commands.registerCommand("name-code-generator.generateNameCode", async () => {
        // 获取当前活动的编辑器
        ...
        const hasSensorsTrackImport = documentText.includes("import { sensorsTrack } from '@/hooks/useSensors'");
        // 查找最后一个 import 语句的位置
        ...
        if (selectedText) {
            ...
            const selectedCategoryItem = await vscode.window.showQuickPick(categoryOptions, {
                placeHolder: "请选择事件类别",
            });
    ...
    { category: '${selectedCategory}' },
        ...
        editor
            ...
            if (!hasSensorsTrackImport) {
                editBuilder.insert(importInsertPosition, "import { sensorsTrack } from '@/hooks/useSensors';\n");
            }
            // 获取下一行的起始位置
